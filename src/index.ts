...
async function main() {
  ...
  const targetDirectory = resolveTargetDirectory(args);
  const root = await ensureDirectory(targetDirectory);
...
function printUsage() {
  console.log(`Usage: bun index.ts [target-directory]

Scan a Next.js project for App Router API routes and print them
in a table similar to Laravel's route:list output.

Arguments:
  target-directory   Optional path to the Next.js project (defaults to current directory)

Examples:
  bun index.ts
  bun index.ts ~/Developer/project/my-next-app`);
}

function resolveTargetDirectory(args: string[]): string {
  const positional = args.find((value) => !value.startsWith("-"));
  if (!positional) return process.cwd();
  return expandHome(positional);
}
...
function renderTable(routes: RouteInfo[]): string {
  ...
  const columnWidths = [
    ...
    Math.max(visibleLength(pathHeader), ...formatted.map((entry) => visibleLength(entry.path))),
    Math.max(visibleLength(fileHeader), ...formatted.map((entry) => visibleLength(entry.file))),
  ];
...
function highlightDynamicSegments(pathLabel: string): string {
  return pathLabel.replace(/:(\w+(?:\*\??)?)/g, (_, name) => PARAM_COLOR(`:${name}`));
}

function buildRow(cells: string[], widths: number[], transform: (value: string) => string = (value) => value): string {
